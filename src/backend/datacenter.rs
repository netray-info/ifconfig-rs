...
mod tests {
    ...
    async fn make_db(content: &str) -> DatacenterRanges {
        let id = COUNTER.fetch_add(1, Ordering::Relaxed);
        let path = std::env::temp_dir().join(format!("ifconfig_test_dc_{}.txt", id));
        std::fs::write(&path, content).unwrap();
        let db = DatacenterRanges::from_file(path.to_str().unwrap()).await.unwrap();
    ...
    async fn empty_file_returns_none() {
        ...
        assert!(DatacenterRanges::from_file(path.to_str().unwrap()).await.is_none());
        let _ = std::fs::remove_file(&path);
    }
