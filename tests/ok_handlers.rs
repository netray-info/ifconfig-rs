...
async fn handle_root_json_has_network() {
    ...
    assert_eq!(status, StatusCode::OK);
    let json: serde_json::Value = serde_json::from_str(&body).unwrap();
    // network object should be present with flat classification fields
    assert!(json["network"]["type"].is_string());
    assert!(json["network"]["infra_type"].is_string());
    ...
    assert!(json["network"]["is_c2"].is_boolean());
    assert!(json["network"]["is_spamhaus"].is_boolean());
}
...
async fn handle_network_plain_cli() {
    ...
    assert!(body.contains("is_datacenter:"));
    assert!(body.contains("is_bot:"));
    assert!(body.contains("is_spamhaus:"));
}
...
async fn handle_network_json() {
    ...
    assert!(json["infra_type"].is_string());
    assert!(json["is_datacenter"].is_boolean());
    assert!(json["is_bot"].is_boolean());
    assert!(json["is_spamhaus"].is_boolean());
}
...
async fn network_classification_propagates_to_json() {
    ...
    assert_ne!(
        ...
        "8.8.8.8 should be classified as cloud/datacenter when data files are loaded, got: {}",
        network_type
    );
}
