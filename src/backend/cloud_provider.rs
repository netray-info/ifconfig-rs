...
mod tests {
    ...
    async fn make_db(jsonl: &str) -> CloudProviderDb {
        let id = COUNTER.fetch_add(1, Ordering::Relaxed);
        let path = std::env::temp_dir().join(format!("ifconfig_test_cloud_{}.jsonl", id));
        std::fs::write(&path, jsonl).unwrap();
        let db = CloudProviderDb::from_file(path.to_str().unwrap()).await.unwrap();
        let _ = std::fs::remove_file(&path);
        db
    }

    #[tokio::test]
    async fn lookup_aws_ip() {
    ...
    #[tokio::test]
    async fn lookup_cloudflare_no_service() {
    ...
    async fn empty_file_returns_none() {
        ...
        assert!(CloudProviderDb::from_file(path.to_str().unwrap()).await.is_none());
        let _ = std::fs::remove_file(&path);
    }
